...
--10.Calcule o valor total das vendas realizadas por cada funcionário.
  ...
  select * from venda;
  select funcionario_id, sum(venda.total_venda) as total_venda
  from venda
...
--13.Calcule o total arrecadado com vendas para cada cliente.
 ...
 select * from venda;
 select cliente_id, sum(total_venda) as total_arrecadado
 from venda
 group by cliente_id
...
--14.Calcule o preço médio dos produtos em estoque.
  select * from produto;
  select nome_produto, round(avg(preco), 2) as preco_medio
  from produto
...
--16.Calcule a média de salários dos funcionários.
  ...
  select round(avg(salario_funcionario), 2) as media_funcionario
  from funcionario;
